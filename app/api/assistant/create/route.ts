...
import { createAssistantPromptNewMay24 } from "@/utils/AssistantCreationFunctions/create_custom_assistant_new";
...
export async function GET(req: NextRequest, res:NextResponse){
  ...
  const openai = new OpenAI({apiKey:idToken});
   try{
    ...
    const AccOverview = await account_overview(username_chess_com);
    ...
    const instruction = createAssistantPromptNewMay24(username_chess_com, stats, chessalyze_text);
    // console.log("WHOLE INSTRUCTION \n :",instruction);
    try {
      
        const assistant = await openai.beta.assistants.create({
          instructions: instruction,
          name: `Chessy | ${username_chess_com}'s personal chess coach`,
          model:'gpt-4o'  
        });
    
        // console.log(assistant);
    
        return NextResponse.json({assistant:{ assistant_id: assistant.id, rating: user_avg_rating, acc_overview: AccOverview}});
      } catch (e) {
        // console.log(e);
